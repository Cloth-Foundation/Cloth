...
`P001` is thrown when the parser sees another module declaration after one has already been declared. Each Cloth Object
may
...
In a module declaration, if the file is in the root source directory, you can either place the file in a directory with
the
same name as the module, or you can name the module `_src`. `_src` will only ever be valid if the file is in the root
source
directory.
